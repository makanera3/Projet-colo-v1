...
export const CreateAnnonce: React.FC = () => {
  ...
  const handlePlanUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      if (e.target.files && e.target.files[0]) {
          try {
              const base64Plan = await convertToBase64(e.target.files[0]);
              setFormData(prev => ({ ...prev, plan: base64Plan }));
          } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if ((formData.surface || 0) <= 0) {
        setError("La surface doit être strictement positive.");
        return;
    }
    ...
    try {
        ...
        setError("Impossible de publier l'annonce. Vérifiez que votre base de données est initialisée.");
    } finally {
         ...
         <form onSubmit={handleSubmit} className="p-8 space-y-12">
            ...
            <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="adresse" value={formData.adresse} onChange={handleChange} className="w-full rounded-xl border-slate-200 py-3 px-4 bg-slate-50" placeholder="10 Rue de la Paix..." />
                    </div>
                     <div>
                    ...
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-2">Disponible le</label>
                        <input type="date" name="dateDisponibilite" value={formData.dateDisponibilite} onChange={handleChange} className="w-full rounded-xl border-slate-200 py-3 px-4 bg-slate-50" />
                    </div>

                    <div className="grid grid-cols-2 gap-6">
                        <div>
                            <label className="block text-sm font-bold text-slate-700 mb-2">Logement meublé ?</label>
                            <select 
                                name="meuble" 
                                value={formData.meuble ? 'true' : 'false'} 
                                onChange={handleChange} 
                                className="w-full rounded-xl border-slate-200 py-3 px-4 bg-slate-50"
                            >
                        ...
                        <div>
                            <label className="block text-sm font-bold text-slate-700 mb-2">Stationnement inclus ?</label>
                            <select 
                                name="parking" 
            ...
            <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-emerald-50/50 rounded-2xl border border-emerald-100">
                    <div>
                         <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Consommation Primaire</label>
                         <div className="flex gap-2">
                             <input type="number" min="0" name="dpeConso" value={formData.dpeConso} onChange={handleChange} className="w-full rounded-xl border-slate-200 py-2 px-3 text-sm" placeholder="kWh/m²/an" />
                             <select name="dpeLettre" value={formData.dpeLettre} onChange={handleChange} className="rounded-xl border-slate-200 py-2 px-3 font-bold">
                                 {['A','B','C','D','E','F','G'].map(l => <option key={l} value={l}>{l}</option>)}
                             </select>
                         </div>
                    </div>
                    <div>
                         <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Émissions GES</label>
                         <div className="flex gap-2">
                             <input type="number" min="0" name="gesEmission" value={formData.gesEmission} onChange={handleChange} className="w-full rounded-xl border-slate-200 py-2 px-3 text-sm" placeholder="kgCO2/m²/an" />
                             <select name="gesLettre" value={formData.gesLettre} onChange={handleChange} className="rounded-xl border-slate-200 py-2 px-3 font-bold">
                                 {['A','B','C','D','E','F','G'].map(l => <option key={l} value={l}>{l}</option>)}
                             </select>
                         </div>
                    </div>
            ...
            <section>
                <h3 className="text-lg font-bold text-slate-900 mb-6 flex items-center gap-2">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                     ...
                     <div>
                         <label className="block text-sm font-bold text-slate-700 mb-2">Charges</label>
                         <div className="relative">
                            <input type="number" min="0" name="charges" value={formData.charges} onChange={handleChange} className="w-full rounded-xl border-slate-200 pl-4 pr-8 py-3 bg-slate-50" />
                            <span className="absolute right-4 top-3 text-slate-400">€</span>
                         </div>
                     </div>
                      <div>
                         <label className="block text-sm font-bold text-slate-700 mb-2">Dépôt de garantie</label>
                         <div className="relative">
                            <input type="number" min="0" name="depotGarantie" value={formData.depotGarantie} onChange={handleChange} className="w-full rounded-xl border-slate-200 pl-4 pr-8 py-3 bg-slate-50" />
                            <span className="absolute right-4 top-3 text-slate-400">€</span>
                         </div>
                     </div>
                 </div>
             ...
             <section>
                <h3 className="text-lg font-bold text-slate-900 mb-6 flex items-center gap-2">
                    <span className="w-8 h-8 rounded-lg bg-pink-100 text-pink-600 flex items-center justify-center text-sm">4</span>
                    Photos & Plans
                </h3>
                        ...
                        <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300 border-dashed rounded-xl hover:bg-slate-50 hover:border-violet-300 transition-colors cursor-pointer relative">
                             ...
                             <div className="space-y-1 text-center pointer-events-none">
                                ...
                                <div className="flex text-sm text-slate-600 justify-center">
                                    <span className="relative font-bold text-violet-600 rounded-md focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-violet-500">
             ...
             <section>
                 <div className="bg-indigo-50 p-6 rounded-2xl border border-indigo-100">
                    ...
                    <textarea 
                        ...
                        placeholder="Parlez-nous du quartier, des colocataires déjà présents, etc..."
                        required
                    ></textarea>
                 </div>
             </section>
